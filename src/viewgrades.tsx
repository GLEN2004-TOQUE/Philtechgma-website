...
const ViewGrades = () => {
  ...
  const fetchUserCourses = async () => {
    try {
      // Fetch user's courses (similar to CollegePortal.tsx)
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) return;
      ...
      // Note: No grades are fetched because teacher hasn't given any yet
      setGrades([]);
    } catch (error) {
        ...
        <div className="px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <button
                onClick={() => navigate("/CollegePortal")}
                className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
              >
                <ArrowLeft size={24} />
              </button>
              <div>
                <h1 className="text-xl font-bold text-gray-900 dark:text-white">Grades Overview</h1>
        ...
        <div className="mb-8">
          ...
          <div className="bg-gradient-to-r from-[#7B1112] to-[#FFB302] rounded-2xl p-8 text-white mb-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between">
              <div>
                <p className="text-white/80 mb-2">Current Cumulative GPA</p>
                <p className="text-5xl font-bold mb-4">{calculateGPA()}</p>
